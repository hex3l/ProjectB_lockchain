...
contract DealHandler {
    ...
    // 
	function createDeal(uint amount, address _target, uint dealID, address _source) public onlyAlive() payable {
	...
	function confirmTargetDeal(uint _id) onlyTarget(_id) public {
	...
	// Target can confirm that the task agreed in deal has been compleeted
	function confirmSourceDeal(uint _id) onlySource(_id) public {
	...
	function fulfillDeal(uint _id) onlyAgreed(_id) private {
